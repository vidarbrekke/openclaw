...
- `ROUND_ROBIN_MODELS` — Round-robin is **opt-in**: type `/round-robin` in chat to enable for that session. Set to `off` to disable the feature entirely. Set to comma-separated model IDs to override. Config file `~/.openclaw/round-robin-models.json` overrides env. Install once with `bash skills/round-robin/install.sh`. See `skills/round-robin/README.md`.
